...
import { fetchGalleryImages, urlFor, SanityGalleryImage } from "@/lib/sanity";
...
const ASPECTS = ["aspect-[4/5]", "aspect-[3/4]", "aspect-square", "aspect-[4/3]", "aspect-square", "aspect-[3/4]", "aspect-[4/5]", "aspect-square"];
...
export default function Gallery() {
  ...
  const [sanityImages, setSanityImages] = useState<SanityGalleryImage[]>([]);

  useEffect(() => {
    fetchGalleryImages().then((data) => {
      if (data && data.length > 0) setSanityImages(data);
    });
  }, []);
            ...
            ? sanityImages.map((img, index) => (
                ...
                >
                  <figure className="relative group overflow-hidden rounded-2xl shadow-sm border bg-card">
                    <div className={`relative w-full ${ASPECTS[index % ASPECTS.length]} overflow-hidden`}>
                      <img
