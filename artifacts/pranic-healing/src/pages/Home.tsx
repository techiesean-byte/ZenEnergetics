...
const DEFAULT_HEADLINE = "Discover the Healing Power Within You";
const DEFAULT_SUBTEXT = "Pranic Healing is a gentle, no-touch energy healing system that guides you from exhaustion to radiant wellbeing.";
const DEFAULT_BIO1 = "Through years of exploring various healing and spiritual modalities, Rosalyn discovered Pranic Healing and non-dualism. These practices have become the foundation of her personal and professional life.";
...
export default function Home() {
  ...
  const [bio2, setBio2] = useState(DEFAULT_BIO2);

  useEffect(() => {
    fetchSiteSettings().then((s) => {
      if (s?.heroHeadline) setHeadline(s.heroHeadline);
      if (s?.heroSubtext) setSubtext(s.heroSubtext);
      if (s?.homeBio1) setBio1(s.homeBio1);
      if (s?.homeBio2) setBio2(s.homeBio2);
    });
    fetchAboutBio().then((bio) => {
      // About bio takes precedence over siteSettings for bio paragraphs if set
      if (bio?.bio1) setBio1(bio.bio1);
      if (bio?.bio2) setBio2(bio.bio2);
    });
  }, []);
    ...
    <div className="flex flex-col items-center w-full">
      ...
      <section className="relative w-full min-h-[90vh] flex items-center justify-center overflow-hidden bg-background">
        <div className="absolute inset-0 z-0">
          <img src={heroImg} alt="Glowing healing energy hands" className="w-full h-full object-cover opacity-80" />
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/55 to-transparent" />
        </div>

        {/* Purple floating orbs */}
        <motion.div className="absolute top-1/4 left-1/4 w-72 h-72 bg-secondary/50 rounded-full blur-[110px] pointer-events-none"
          animate={{ x: [0, 50, 0], y: [0, -50, 0], scale: [1, 1.2, 1] }}
          transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute bottom-1/3 right-1/4 w-96 h-96 bg-primary/15 rounded-full blur-[130px] pointer-events-none"
          animate={{ x: [0, -50, 0], y: [0, 50, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 20, repeat: Infinity, ease: "easeInOut", delay: 2 }} />
        <motion.div className="absolute top-1/2 right-1/3 w-56 h-56 bg-secondary/40 rounded-full blur-[90px] pointer-events-none"
      ...
      <section className="w-full py-24 bg-muted/60">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: Heart, color: "bg-primary/10 text-primary", title: "Healing Sessions", body: "Experience deep restoration through one-on-one Pranic Healing. Available in-person or remotely.", link: "/services", label: "View Services", delay: 0.1 },
              { icon: Leaf, color: "bg-secondary text-secondary-foreground", title: "Learn & Grow", body: "Understand the anatomy of the energy body, the chakras, and how to maintain energetic hygiene.", link: "/faq", label: "Read FAQ", delay: 0.2 },
              { icon: Sparkles, color: "bg-secondary/60 text-secondary-foreground", title: "Video Library", body: "Guided meditations and educational videos to help you align your energy from the comfort of home.", link: "/videos", label: "Watch Now", delay: 0.3 },
              ...
              <motion.div key={title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <Card className="h-full border border-secondary/40 shadow-md hover:shadow-xl transition-shadow bg-card/70 backdrop-blur-sm">
                  ...
                  <CardContent><p className="text-muted-foreground">{body}</p></CardContent>
                  <CardFooter>
                    <Link href={link} className="text-primary font-medium inline-flex items-center hover:underline"
                      data-testid={`link-path-${label.toLowerCase().replace(/\s/g, "-")}`}>
          ...
          <div className="grid md:grid-cols-2 gap-14 items-center max-w-5xl mx-auto">
            <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }}
              ...
              <div className="rounded-2xl overflow-hidden shadow-2xl ring-4 ring-secondary/40">
                <img src={healerPhoto} alt="Rosalyn Piza — Pranic Healer"
                  className="w-full h-[480px] object-cover object-top" data-testid="img-healer-home" />
              </div>
              <motion.div className="absolute -bottom-5 -right-5 bg-card border border-secondary/50 rounded-xl px-5 py-4 shadow-lg"
            ...
            <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }}
              ...
              <h2 className="font-serif text-4xl md:text-5xl font-light text-foreground leading-tight">Rosalyn Piza</h2>
              <p className="text-sm text-muted-foreground font-medium tracking-wide">Pranic Healer · Non-Dualism</p>
              <p className="text-foreground/80 leading-relaxed text-lg">{bio1}</p>
